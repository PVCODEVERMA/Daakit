...
<div class="page-header main-container container-fluid px-5">
    ...
    <ol class="breadcrumb">
        ...
        <?php if (in_array('ndr_export', $user_details->permissions)) { ?>		
            <a href="<?= base_url('admin/ndr/exportCSV'); ?><?php if (!empty($filter)) { echo "?" . http_build_query($_POST); } ?>" class="btn btn-info btn-sm pull-left display-block mright5" style="margin-right: 10px;"> Export </a>
             <?php } ?>
				...
				<div class="chat">
					<div class="contacts_card">
						<div class="input-group p-3">
                            <form method="post" action="<?= base_url('admin/ndr') ?>" id="form_filter">
                                <div class="row">
                                    <div class="col-md-6">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group" app-field-wrapper="from_date">
                                            <label for="from_date" class="control-label">NDR ID(s)</label>
                                            <input type="text" autocomplete="off" name="filter[ndr_ids]" value="<?= !empty($filter['ndr_ids']) ? $filter['ndr_ids'] : '' ?>" class="form-control" placeholder="Enter ndr ids separated by comma">                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group" app-field-wrapper="from_date">
                                    ...
                                    <div class="form-group col-sm-12" style="margin-top:2px;">
                                            ...
                                            <select name="filter[seller_id]" class="form-control getUserlist" style="width: 100% !important;">
                                        ...
                                        <div class="form-group col-sm-12">
                                            ...
                                            <select name="filter[attempts]" class="form-control js-select2" style="width: 100% !important;">
                                                <?php
                                                $attempts = '';
                                        ...
                                        <div class="form-group col-sm-12">
                                            <label class="control-label">NDR Type:</label>
                                            <select name="filter[ndr_type]" class="form-control js-select2" style="width: 100% !important;">
                                                <?php
                                                $ndr_type = '';
                                                if (!empty($filter['ndr_type']))
                                                    $ndr_type = $filter['ndr_type'];
                                                ?>
                                                <option <?php if ($ndr_type == '') { ?> selected <?php } ?> value="">All</option>
                                                <option <?php if ($ndr_type == 'fake') { ?> selected <?php } ?> value="fake">Fake</option>
                                                <option <?php if ($ndr_type == 'genuine') { ?> selected <?php } ?> value="genuine">Genuine</option>

                                            </select>
                                        </div>
                                        <div class="form-group col-sm-12" style="margin-top:2px;">
                                            <label class="control-label">Multiple Account Manager </label>
                                ...
                                <div class="row">
                                    <div class="col-md-6" style="margin-top:29px;">
                                        <div class="form-group" app-field-wrapper="to_date">
                                            ...
                                            <a href="<?= base_url('admin/ndr'); ?>" class="btn btn-sm btn-primary">Reset</a>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        ...
                        <div class="btn btn-sm btn-success ms-auto">
                            <div class="item-action dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="javascript:void(0)" class="dropdown-item fill_bulk_ndr"  data-bs-toggle="modal" data-bs-target="#ndr_submit_model">Bulk NDR</a>
                                </div>
                            </div>
                        </div>
                        </div>
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php echo ucwords($ndr->user_fname . ' ' . $ndr->user_lname); ?><br>
                                            <?php echo ucwords($ndr->sellercompany); ?>
                                        </td>
                                        <td>
                                            <?php
                                        ...
                                        <td>
                                            ...
                                            <a target="blank" class="text-info" href="<?php echo base_url('awb/tracking/')?><?= $ndr->awb_number ?>"><?= ucwords($ndr->awb_number); ?></a>
                                            <br>
                        ...
                        <div class="modal-dialog" role="document" style="width: 415px !important;">
                            <div class="modal-content" style="height:auto;">
                                ...
                                <div class="modal-body">
                                    <form method="post" action="<?= base_url('admin/ndr/action'); ?>" id="ndr_submit_form">
                                        <div class="row">
                                            <div class="col-lg-12">
                                                <div class="card-body">
...
<form id="tab_filter" action="<?= base_url('admin/ndr');?>" method="POST">
...
document.addEventListener("DOMContentLoaded", function () {
    const bulkNdrBtn = document.querySelector(".fill_bulk_ndr");
    const ndrIdInput = document.getElementById("ndr_id");
    const checkboxes = document.querySelectorAll(".multiple_checkboxes");

    bulkNdrBtn.addEventListener("click", function (event) {
        event.preventDefault();

        let ndrIds = [];
        checkboxes.forEach(function (cb) {
            if (cb.checked) {
                ndrIds.push(cb.value);
            }
        });

        if (ndrIds.length < 1) {
            showBootstrapAlert("Please select at least one record.");
            // Prevent modal from opening
            const modalEl = document.querySelector("#ndr_submit_model");
            const modal = bootstrap.Modal.getInstance(modalEl);
            if (modal) modal.hide();
            return false;
        }

        ndrIdInput.value = ndrIds.join(",");
    });

    // Optional: Bootstrap alert function
