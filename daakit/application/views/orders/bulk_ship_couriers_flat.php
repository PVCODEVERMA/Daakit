<script>
    $(".bulk_ship_form").submit(function(event) {
        ...
        var courier_id = $("input[name='courier_id']:checked").val();
        $.ajax({
            url: 'orders/bulk_ship',
            type: "POST",
            data: {
                order_ids: orderids,
...
<form class="bulk_ship_form" method="post">
    <div class="modal-header">
        <h5 class="modal-title" id="mySmallModalLabel">Select Your Preferred Courier Company</h5>
    </div>
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <select class="form-control" name="warehouse_id" id="select_warehouse_dropdown_bulk">
            ...
            <div class="col-sm-6">
                ...
                <div class="form-group">
                    ...
                    <div>
                        <select class="form-control" name="rto_warehouse_id" id="select_rto_warehouse_dropdown_bulk">
                            <?php foreach ($warehouses as $warehouse) { ?>
                            <option <?php if ($warehouse->id == $selected_warehouse) { ?> selected="" <?php } ?> value="<?= $warehouse->id; ?>"><?= ucwords($warehouse->name); ?><?php if ($warehouse->is_default) { ?> (Default) <?php } ?></option>
                            <?php } ?>
                        </select>
            ...
            <div class="col-sm-6 couriourval  <?php  if(isset($courier_type) and $courier_type!='') { echo strtolower($courier_type); } else { echo "";}  ;?>   <?php  if(isset($courier_weight) and $courier_weight!='') { echo $courier_weight; } else { echo "";}   if(isset($courier->prefered) && $courier->prefered=='1' )  { echo " prefered";} else { echo " notprefered";}  ?> "  data-target="<?php  if(isset($courier->courier_type) and $courier->courier_type!='') { echo $courier->courier_type; } else { echo "-";}  ;?> ">
                <div class="alert alert-success fade show" role="alert">
                    <div class="custom-control custom-radio">
                        ...
                        <label class="custom-control-label" for="customRadio<?= ucwords($courier->courier); ?>"><?= ucwords($courier_type) . ' ' . round($courier_weight/1000, 2) . 'kg'; ?></label>
                    </div>
                </div>
            </div>
            <?php } ?>
        ...
        <div class="col-sm-6">
         ...
         <!--<div class="row border-bottom m-b-10">
            <div class="col-sm-6">
                <div class="alert alert-info fade show" role="alert">
                    <div class="custom-control custom-checkbox custom-control-inline">
                        <input type="checkbox" name="essential_order" class="custom-control-input" id="essential_order" value="1">
                        <label class="custom-control-label" for="essential_order">Check this if orders contain essential items</label>
                    </div>
                </div>
            </div>
        </div>-->
