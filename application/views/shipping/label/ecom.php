    ...
    <tbody>
        <tr>
            <td style="margin-top: 0px;display: block; width: 60%">
                ...
                <?php } ?>
                <p><?= str_replace(array('<','>',', ,'), array(' ',' ',','), ucwords($order_info->shipping_address . ', ' . $order_info->shipping_address_2 . ', ' . $order_info->shipping_city . ', ' . $order_info->shipping_state . ', ' . $order_info->shipping_country . ' - ' . $order_info->shipping_zip)); ?></p>
    ...
    <tbody>
        <tr>
            <td colspan="2" style="text-align:center;">
                <?php if(!empty($courier->delivery_area_code)) { ?><h3>[<?= $courier->delivery_area_code; ?>]</h3><?php } ?>
            </td>
        </tr>
        <tr>
...
<table style="width: 100%;padding: 5px;border-collapse: collapse;">
    ...
    <tbody>
        ...
        <?php if (!empty($order_info->shipping_charges) && $order_info->shipping_charges > 0) { ?>
            <tr>
                <td style="text-align:center;border: 1px solid black;">
                    <?= !empty($product->product_sku) ? strtoupper(wordwrap($product->product_sku, 12, "<br>\n", true)) : 'N/A'; ?>
                </td>
                <td style="text-align:center;border: 1px solid black;">
                    Shipping Charges
                </td>
                ...
                <td style="text-align:center;border: 1px solid black;">
                    Rs. <?= round($order_info->shipping_charges, 2); ?>
                </td>
            </tr>
        <?php } ?>
